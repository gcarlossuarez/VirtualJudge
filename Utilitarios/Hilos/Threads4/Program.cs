...
class Program {
    static void Main() {
        ...
        Console.Write("Presione 's' para detener los hilos:");
        string? detener = Console.ReadLine();
        if(detener?.Trim().ToLower() == "s")
        {
            Console.WriteLine("Deteniendo hilos...");
            foreach (Thread thread in threads) 
            {
                thread.Abort(); // Detiene el hilo a la fuerza (no recomendad, salvo casos extremos). NOTA.- No soportada en esta paltaforma.
            }
        }
