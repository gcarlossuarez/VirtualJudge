// Las Oleadas Sincronizadas de Vlad
// LCS
using System;
...
class Program
{
    ...
    public static int[][] SolveLCS(int M, int N, List<string> norte, List<string> sur)
    {
        int [][] answer = new int[N + 1][];
	//Console.WriteLine($"M:{M}. N:{N}. norte:{string.Join(",", norte)}. sur:{string.Join(",", sur)}");
	...
	// Cargar la matriz de dp en la estructura de datos con la respuesta esperada
        for(int i = 0; i <= N; ++i)
        {
    ...
    static void Main()
    {
        ...
        int [][] answer = SolveLCS(M, N, norte, sur);
