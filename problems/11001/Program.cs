...
class Program
{
    ...
    static void Main()
    {
        ...
        /*
		Console.WriteLine("Una selección válida es:");

        // ================================
        // 6) Reconstruir solución
        // ================================
        var seleccion = new List<Plato>();

        int resto = presupuesto;

        for (int i = n; i >= 1; i--)
        {
            if (dp[i - 1, resto])
            {
                // No se tomó este plato
                continue;
            }
            else
            {
                // Sí se tomó este plato
                seleccion.Add(platos[i - 1]);
                resto -= platos[i - 1].Costo;
            }
        }
        ...
        // 7) Mostrar resultado
        // ================================
        seleccion.Reverse(); // opcional

        foreach (var p in seleccion)
		{
            Console.WriteLine($"{p.Nombre};{p.Costo}");
		}
		*/
    }
}
